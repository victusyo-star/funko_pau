...
(() => {
  ...
  async function review(){
    ...
    const list=changes();
    open(t('Review your update','Revisar tu actualización'),`<p>${list.length} ${t('changed entries. Nothing has been published.','entradas modificadas. No se ha publicado nada.')}</p><ul class="pau-editor-summary">${list.map(c=>`<li>${escape(c.label)} — ${c.kind==='figure'?t('Collection details','Datos de colección'):c.before?t('Contributor updated','Colaborador actualizado'):t('New contributor','Nuevo colaborador')}</li>`).join('')}</ul><p>${t('Download the ZIP, then send it through your usual email or messaging app to the adult who publishes the collection. Keep your draft until the update is published.','Descarga el ZIP y envíalo por tu correo o aplicación habitual al adulto que publica la colección. Conserva el borrador hasta que se publique la actualización.')}</p><p id="pau-source-status" role="status"></p><div class="pau-editor-actions">${button(t('Download update','Descargar actualización'),'export',true,!list.length)}${button(t('Back to editing','Volver a editar'),'close')}</div><p id="pau-export-status" role="status"></p>`);
  }
  ...
  async function exportUpdate(){
    ...
    const exportButton=dialog.querySelector('[data-pau="export"]');exportButton.disabled=true;
    try{
      validate(draft);
      ...
      const blob=zip(files),name='Funko-Pau-update-'+date.replace(/[:.]/g,'-')+'.zip';pendingPackage={blob,name};download(blob,name);
      $('#pau-source-status').textContent='';$('#pau-export-status').textContent=t('Download requested. Send the ZIP to your publisher. Your draft is retained; nothing was sent or published automatically.','Descarga iniciada. Envía el ZIP al responsable de publicar. Tu borrador se conserva; no se ha enviado ni publicado nada automáticamente.');
      try{const file=new File([blob],name,{type:'application/zip'});if(navigator.canShare?.({files:[file]}))$('#pau-export-status').insertAdjacentHTML('afterend',button(t('Share downloaded update','Compartir actualización'),'share'));}catch{/* Download remains available when native sharing is unsupported. */}
    }catch{error(t('Could not verify or export the update. Your draft is retained. Check your connection, or download a backup.','No se pudo verificar o exportar. Tu borrador se conserva. Revisa la conexión o descarga una copia.'));}
  ...
  async function action(name){
    ...
    if(name==='export')await exportUpdate();
    if(name==='share'&&pendingPackage){try{await navigator.share({files:[new File([pendingPackage.blob],pendingPackage.name,{type:'application/zip'})],title:'Funko Pau'});toast(t('Opened sharing. Complete sending in your chosen app.','Se abrió el menú de compartir. Completa el envío en tu aplicación.'));}catch{toast(t('Sharing cancelled or unavailable. Use the downloaded ZIP.','Envío cancelado o no disponible. Usa el ZIP descargado.'));}}
  }
